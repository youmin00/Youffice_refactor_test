...
def _render_actionable_project_guidance(
    ...
    *,
    ...
    lines = [
        "[프로젝트 준비 결론]",
        (
            "팀이 만든 결과는 완성품이 아니라, 사용자가 실제 제작·개발을 시작하기 위한 프로젝트 준비서입니다. "
            f"현재 상태는 '{review_status}'입니다. 이는 팀이 작성한 문서의 누락·모순을 검토한 결과이며, "
            "실제 구매, CAD 도면 작성, 조립, 코드 실행, 성능·안전 시험이 끝났다는 뜻은 아닙니다."
        ),
        "",
        "[준비된 선택지와 제안]",
        "아래 내용은 확인된 요구사항과 구분된 제안이며, 실제 작업의 다음 결정을 돕기 위한 자료입니다.",
    ]
    if proposals:
    ...
    lines.extend([
        "",
        "[사용자가 이어서 할 일]",
        "1. 위 조건을 선택하거나 기본 가정으로 유지할지 정합니다.",
        "2. 부품·도구·호환 조건은 원문 출처와 실제 판매 정보를 다시 확인합니다.",
        "3. 필요한 경우 CAD·개발 도구에서 설계와 코드를 만들고, 준비서의 시험 기준으로 실제 결과를 기록합니다.",
        "",
        "[준비서의 검토 범위와 남은 확인]",
        f"- 프로젝트 준비서 검토 상태: {review_status}",
        "- 실행 증거가 없는 구매·도면·코딩·조립·성능 시험은 완료로 처리하지 않습니다.",
    ])
    if limitations:
        lines.append("- 추가로 확인할 점:")
        lines.extend(f"  - {item}" for item in limitations)
    else:
        lines.append("- 위의 첫 결정을 정한 뒤 외부 도구에서 실제 설계·제작·개발을 시작할 수 있어요.")
    return "\n".join(lines)

...
EMPLOYEE_WORKSTREAM_LABELS = {
    "manager": "팀장·준비 계획 조율",
    "memory": "기억·요구사항 확인",
    "planning": "기획·일정·선택 기준",
    "technical": "기술 요구사항·제작 준비",
    "review": "위험·누락·실현 가능성 검토",
    "report": "프로젝트 준비서 정리",
    "general": "프로필 지정 업무",
}
...
EMPLOYEE_ASSIGNMENT_CONTRACTS = {
    ...
    "planning": {
        "objective": "사용자가 실제 작업을 시작할 수 있도록 범위, 선택 기준, 일정과 다음 행동을 기획한다.",
        "deliverables": (
            "사용자가 제공한 요구사항과 우선순위",
            "조사·설계·제작·시험을 구분한 실행 순서",
    ...
    "technical": {
        "objective": "확인된 요구사항을 외부 설계·개발 도구로 넘길 수 있는 기술 준비 자료로 변환한다.",
        "deliverables": (
            "현재 자료로 설명할 수 있는 기능 구성과 인터페이스",
            "부품·소프트웨어 후보, 선택 기준과 호환성 확인 항목",
            "CAD·코딩·조립·시험 전에 정할 조건과 외부 도구 전달 체크리스트",
        ),
        "forbidden": (
            "프로젝트 전체 일정과 예산 다시 작성하기",
            "자기 결과를 검수 통과시키거나 최종 보고서 작성하기",
            "실행 증거 없이 도면·코드·조립·성능 시험이 완료됐다고 표현하기",
        ),
    },
    "review": {
        "objective": "제작 준비 자료의 오류, 누락, 위험, 근거와 실제 실행 전 확인점을 독립적으로 검토한다.",
        "deliverables": (
            "통과 또는 재작업 판정",
            "판정 근거와 수정이 필요한 항목",
            "안전 조건과 확인되지 않은 주장",
    ...
    "report": {
        "objective": "직원 결과와 검수 기록을 사용자가 바로 이어서 행동할 수 있는 프로젝트 준비서로 통합한다.",
        "deliverables": (
            "확정 요구사항, 선택 근거와 팀의 핵심 준비 결과",
            "사용자가 직접 할 일, 외부 도구로 넘길 일과 테스트 순서",
            "기본 가정·미확인 사항·실제 미실행 범위가 분명한 한국어 준비서",
        ),
        "forbidden": (
...
def build_manager_delegation_instruction(
    ...
    return (
        ...
        f"{roster}\n"
        "출력은 반드시 다음 순서를 사용한다.\n"
        "[업무 접수] 만들려는 결과, 현재 확정 조건과 기본 가정\n"
        "[직원별 배정] 제작 준비를 위한 각 직원의 담당 범위와 산출물\n"
        "직원별 배정에서는 직원마다 `- **이름 (직책):**`를 독립된 한 줄에 쓰고, "
        "해당 직원의 업무와 산출물은 바로 아래에 네 칸 들여쓴 `    -` 하위 목록으로 작성한다. "
        "직원 이름과 업무 내용을 같은 단계의 목록으로 섞지 않는다.\n"
        "[작업 순서] 기억 확인 → 기획·조사 → 기술 준비 → 위험 검토 → 프로젝트 준비서의 전달 순서\n"
        "[승인 요청] 사용자가 승인하면 직원 AI가 제작 준비 자료 작성과 상호 검토를 시작한다는 안내\n"
        "승인 전에는 구체적인 부품 목록, 코드, 완성 설계, 검수 결과 또는 최종 답을 미리 만들지 않는다. "
        "AI가 실제 구매·CAD 도면 작성·조립·코드 실행·성능 시험을 수행한다고 약속하지 않으며, "
        "없는 수치나 조건을 추정하지 않는다. /no_think\n"
    )
